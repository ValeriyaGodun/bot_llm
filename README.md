...
- `llm_bot.py` - точка входа и вся логика Telegram: обработчики команд, вызов модели.
- `conversation_utils.py` - управление историей диалога, конфигурацией и ограничениями по токенам.
...
Когда запускается функция `load_config()`, она берёт значения параметров из переменных окружения (ENV), а затем передаёт их в dataclass `BotConfig`. Таким образом, все параметры конфигурации бота (например, токены, имя модели, ограничения истории) собираются в одном объекте, что упрощает настройку и использование этих данных по всему коду.
...
* `history_max_pairs`, `history_max_tokens`, `history_ttl_seconds` - ограничения истории и её время жизни (TTL);
* `model_device` оставлен для сценария, когда модель будет запускаться локально.

## Архитектура и потоки данных

1. Пользователь отправляет сообщение - `python-telegram-bot` вызывает `handle_message`.
2. `handle_message` проверяет текст, показывает индикатор набора (`ChatAction.TYPING`) и передаёт задачу: `loop.run_in_executor(None, generate_response, ...)`. 
3. Функция `generate_response` запускается в отдельном потоке, чтобы не блокировать основной, пока идёт запрос к Hugging Face.`generate_response`:
   ...
   - синхронно обращается к `InferenceClient.chat.completions.create`;
   - очищает ответ от `<think>` и сохраняет  реплику ассистента в историю.
4. Telegram-бот отправляет ответ пользователю.

## ConversationManager — контроль истории

Файл `conversation_utils.py`содержит логику работы с историей:

- `purge_inactive()` выбрасывает диалоги старше заданного TTL, чтобы не расходовать память.
...
- `build_prompt()` и `build_prompt_from_history()` умеют работать как с `tokenizer.apply_chat_template`, так и с собственным форматом `<|role|>`.

Таким образом, при любой длине переписки модель получает «чистый» и не превышающий лимиты контекст.
...
Далее используется метод `chat.completions.create` с параметрами из `BotConfig`. При желании можно заменить модель, передав другое имя через ENV.

Чтобы скрыть «глубокое мышление» моделей, введена функция `_clean_model_output`, которая срезает блоки `<think>...</think>`.
